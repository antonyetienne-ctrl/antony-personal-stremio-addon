...
<section><h2>🎬 Films</h2><div class="grid"><div><label>Note TMDB minimale</label><input type="number" step="0.1" min="0" max="10" name="movie.minRating" value="${s.movie.minRating}"></div><div><label>Votes minimum</label><input type="number" min="0" name="movie.minVotes" value="${s.movie.minVotes}"></div><div><label>Durée minimale (min)</label><input type="number" min="0" name="movie.minRuntime" value="${s.movie.minRuntime}"></div><div><label>Ordre du catalogue</label><select name="movie.order">${opt('score', s.movie.order)}${opt('random', s.movie.order)}</select></div></div>
<label>Genres à exclure</label>${genreBoxes('movie', MOVIE_GENRES, s.movie.exclude)}</section>
<section><h2>📺 Séries</h2><div class="grid"><div><label>Note TMDB minimale</label><input type="number" step="0.1" min="0" max="10" name="series.minRating" value="${s.series.minRating}"></div><div><label>Votes minimum</label><input type="number" min="0" name="series.minVotes" value="${s.series.minVotes}"></div><div><label>Ordre du catalogue</label><select name="series.order">${opt('score', s.series.order)}${opt('random', s.series.order)}</select></div></div>
<label>Genres à exclure</label>${genreBoxes('series', TV_GENRES, s.series.exclude)}<div class="muted">TMDB n'a pas de genre Horreur/Romance/Musique pour les séries : ces trois cases détectent par mots-clés.</div></section>
...
<label class="chk"><input type="checkbox" name="common.excludeKids" value="1" ${s.common.excludeKids ? 'checked' : ''}>Exclure les contenus pour enfants</label>
<label class="chk"><input type="checkbox" name="common.excludeWesternKidsAnimation" value="1" ${s.common.excludeWesternKidsAnimation ? 'checked' : ''}>Exclure l'animation occidentale pour enfants (anime autorisé)</label>
...
st.innerHTML=h;setTimeout(poll,s.running?4000:15000)}catch(e){st.textContent='statut indisponible';setTimeout(poll,15000)}}
document.getElementById('rb').onclick=async()=>{const r=await fetch('/u/'+id+'/rebuild',{method:'POST'});const j=await r.json();st.textContent=j.started?'⏳ Calcul lancé…':'ℹ️ '+(j.why||'déjà en cours');poll()};poll();</script>` : ''}
</body></html>`;
}
module.exports = { page };
