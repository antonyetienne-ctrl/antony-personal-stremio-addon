...
function defaultSettings() {
  ...
  return {
    ...
    common: { excludeCancelled: true, movieCatalog: true, seriesCatalog: true, libMovieCatalog: true, libSeriesCatalog: true, frMeta: true, useGemini: true, newSeasonsCatalog: true, semanticRescue: false, catalogOrder: CATALOG_IDS.slice(), install: defaultInstall() }
  };
}
...
function normalizeSettings(input = {}, prev = defaultSettings()) {
  ...
  for (const k of ['excludeCancelled', 'movieCatalog', 'seriesCatalog', 'libMovieCatalog', 'libSeriesCatalog', 'newSeasonsCatalog', 'semanticRescue', 'frMeta', 'useGemini']) if (k in c) p.common[k] = Boolean(c[k]);
...
const key = {
  ...
  imdb: `${NS}:imdbr`,
  vf: `${NS}:vf`,
  gemab: (id) => `${NS}:gemab:${id}`,                    // grand test de Gemini (cartes et réponses)
