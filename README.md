...
## 7.9.0 — version unique regroupant tout ce qui a été validé après la 7.8.0
- **Nouvelles Saisons corrigées** : les vraies listes de Stremio ont été décodées (*Outlast : La jungle*, *Pluribus*, *Alice in Borderland*) : quand une saison ou une série est marquée « vue » en une fois, l'ancre n'est pas le dernier bit à 1 ; la lecture stricte refusait ces listes et la série restait affichée. Le contrôle accepte maintenant cette forme (bit de l'ancre à 1) et garde la forme stricte prioritaire. La saison 1 d'une série déjà vue n'est jamais « nouvelle » (seules les saisons ≥ 2). Une saison est « entièrement vue » quand tous ses épisodes SORTIS le sont (les épisodes seulement annoncés ne comptent plus) ; une saison annoncée pour plus tard est ignorée.
- **Mémoire (le serveur plantait avec beaucoup de candidats)** : aucun candidat n'est plafonné. Chaque candidat noté libère aussitôt ses caches (tables de similarités, caractéristiques recalculables) ; `src/memory.js` surveille le tas après chaque étape lourde et vide les caches au-delà de 70 % de la limite (ramasse-miettes demandé avec `--expose-gc`) ; `Dockerfile` : `NODE_OPTIONS=--max-old-space-size=400` et `node --expose-gc` ; la reprise automatique d'un calcul interrompu s'arrête après 2 interruptions consécutives au même endroit (les listes déjà publiées restent servies).
- **Découverte complète** : `PAGE_CAP` 500 (maximum de TMDB), budget d'appels `DISCOVER_CALL_BUDGET` 900 par type et par calcul ; l'univers n'est plus tronqué à 5 000 titres.
- **« Pourquoi » supprimés** : plus de texte dans la fiche, plus de points de chances sur les cartes, plus de ressource `stream`, plus de réglages `whyMeta` / `whyCards` / `whyStream`. La page « recheck » est conservée (avec un lien depuis la page de réglages).
- **Validation croisée** : tranche de 10 % du haut de liste au lieu de 3 %, calibration isotone du risque estimé, et un réglage n'est adopté que si son gain a un intervalle de confiance au-dessus de zéro.
- **Grand test de Gemini** (`src/gemab.js`, mesure seulement) : 600 titres équilibrés (films et séries × ❤️, 👍, vu sans note), voisins tirés du reste de l'historique, mêmes voisins pour les deux variantes (A = sans 👍, prompt de la 7.7.0 ; B = avec les 2 titres 👍 les plus proches), chaque variante jouée deux fois, analyse appariée avec intervalle à 95 %. Reprise possible d'un jour à l'autre (quota). Résultat dans le diagnostic.
- **Signaux supplémentaires** (`src/signals.js`, variantes E, F, G du backtest, adoptés seulement s'ils gagnent) : personnes (réalisateur, compositeur, 5 premiers acteurs, identifiants TMDB seulement, au moins 3 titres de l'historique) ; recommandations des autres (❤️ et 👍 seulement, jamais les vus sans note, titre lui-même exclu).
- Le repêchage sémantique reste décoché ; il est remesuré à chaque validation croisée et sera supprimé s'il ne devient pas positif après 1 à 3 versions.
